...
bool bin_for_best_reset(dataBase_ptr db, GainBucket& gb, AllBin& allbin){
    //best 상태로 옮겨주기
    for(int i=0; i<db->instanceDB->numInsts; ++i){
        bin_for_best_die_move(db->instanceDB->inst_array[i], db, gb, allbin);
    }
    //best 상태에서 gain 계산
    for(int i=0; i<db->instanceDB->numInsts; ++i){
...
void overflow_fm(AllBin& allbin, dataBase_ptr db, GainBucket& gb, double base_overflow){
    ...
    while(!overflow_condition(allbin, base_overflow)){
        ...
        //cout<<"fm count: "<<count<<endl;
    }

    //cout<<"point 8"<<endl;

    bin_for_best_reset(db, gb, allbin);
...
void bin_bi_partition(AllBin& allbin, dataBase_ptr db, GainBucket& gb, double base_overflow){
    ...
    while(best > gb.min_cut_num){
        ...
        cout<<"path-count: "<<count<<", overflow: "<<allbin.ovf_avg[0]<<"/"<<allbin.ovf_avg[1]<<endl;
    }
}
...
void bin_FM(dataBase_ptr db, int bin_num_x, int bin_num_y, double base_overflow){
    ...
    double _base_overflow = (allbin.ovf_avg[0]+allbin.ovf_avg[1])/2;
    cout<<"base_overflow: "<<_base_overflow<<endl;
